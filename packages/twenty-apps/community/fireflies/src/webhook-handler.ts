...
export class WebhookHandler {
    ...
    try {
      ...
      if (!firefliesApiKey) {
        this.logger.critical(
          'FIREFLIES_API_KEY not configured - this is a critical configuration error',
        );
        throw new Error('FIREFLIES_API_KEY environment variable is required');
      }
      if (!twentyApiKey) {
        this.logger.critical(
          'TWENTY_API_KEY not configured - this is a critical configuration error',
        );
      ...
      this.logger.debug(
        `payload meetingId=${payload.meetingId} eventType="${payload.eventType}"`,
      );
      ...
      this.logger.debug(
        `summary strategy: ${summaryConfig.strategy} (retryAttempts=${summaryConfig.retryAttempts}, retryDelay=${summaryConfig.retryDelay}ms)`,
      );
      ...
      const { data: meetingData, summaryReady } =
        await firefliesClient.fetchMeetingDataWithRetry(
          payload.meetingId,
          summaryConfig,
          firefliesPlan,
        );
      ...
      this.logger.debug(
        `meeting data fetched: title="${meetingData.title}" summaryReady=${summaryReady}`,
      );
      ...
      const existingMeetingById = await twentyService.findMeetingByFirefliesId(
        meetingData.id,
      );
      if (existingMeetingById) {
        this.logger.debug(
          `meeting already exists by firefliesMeetingId id=${existingMeetingById.id}`,
        );
        result.success = true;
      ...
      const existingMeetingByTitle = await twentyService.findExistingMeeting(
        meetingData.title,
      );
      if (existingMeetingByTitle) {
        this.logger.debug(
          `meeting already exists by title id=${existingMeetingByTitle.id}`,
        );
        result.success = true;
      ...
      this.logger.debug(
        `total participants from API: ${meetingData.participants.length}`,
      );
      meetingData.participants.forEach((p, idx) => {
        this.logger.debug(
          `participant ${idx + 1}: name="${p.name}" email="${p.email || 'none'}"`,
        );
      });

      const { matchedContacts, unmatchedParticipants } =
        await twentyService.matchParticipantsToContacts(
          meetingData.participants,
        );
      this.logger.debug(
        `matched=${matchedContacts.length} unmatched=${unmatchedParticipants.length}`,
      );

      unmatchedParticipants.forEach((p, idx) => {
        this.logger.debug(
          `unmatched ${idx + 1}: name="${p.name}" email="${p.email || 'none'}"`,
        );
      });
      ...
      this.logger.debug(
        `autoCreate=${autoCreate} createdContacts=${newContactIds.length}`,
      );
      ...
      this.logger.debug(
        `meeting duration: ${meetingData.duration} min (raw from API) → ${meetingInput.duration} min (rounded)`,
      );
      result.meetingId = await twentyService.createMeeting(meetingInput);
      this.logger.debug(
        `created meeting id=${result.meetingId} with noteId=${noteId}`,
      );

      // 9) Link note to participants (Meeting link is handled via the relation field)
      await this.linkNoteToParticipants(twentyService, noteId, allContactIds);
  ...
  private parsePayload(params: unknown): {
    ...
    if (typeof normalizedParams === 'string') {
      this.logger.debug(
        `received params as string length=${normalizedParams.length}`,
      );
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          this.logger.debug(
            `parsed params keys: ${parsedKeys.join(',') || 'none'}`,
          );
        }
      } catch (parseError) {
    ...
    if (isValidFirefliesPayload(normalizedParams)) {
      ...
      if (
        wrapper.headers &&
        typeof wrapper.headers === 'object' &&
        !Array.isArray(wrapper.headers)
      ) {
        extractedHeaders = wrapper.headers as Record<string, string>;
        const headerKeys = Object.keys(extractedHeaders);
        this.logger.debug(
          `extracted headers from wrapper: ${headerKeys.join(',')}`,
        );
      }
  ...
  private verifySignature(
    ...
    const signature =
      (typeof headerSignature === 'string' ? headerSignature : undefined) ||
      payloadSignature;
    ...
    if (!signatureCheck.isValid) {
      ...
      this.logger.critical(
        'Invalid webhook signature - potential security threat detected in production',
      );
  ...
  private async linkNoteToParticipants(
    ...
    for (const contactId of contactIds) {
      try {
        ...
        this.logger.error(
          `failed to link note to person ${contactId}: ${message}`,
        );
      }
    }
  }

  private async createFailedMeetingRecord(
    params: unknown,
    error: string,
  ): Promise<void> {
    try {
      ...
      if (!twentyApiKey) {
        this.logger.debug(
          'Cannot create failed meeting record: TWENTY_API_KEY not configured',
        );
        return;
      }
      ...
      if (payload?.meetingId) {
        ...
        if (firefliesApiKey) {
          try {
            const firefliesClient = new FirefliesApiClient(firefliesApiKey);
            const meetingData =
              await firefliesClient.fetchMeetingData(meetingId);
      ...
      const failedMeetingId =
        await twentyService.createFailedMeeting(failedMeetingData);
