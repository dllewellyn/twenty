...
export class FirefliesApiClient {
  ...
  constructor(apiKey: string) {
    if (!apiKey) {
      logger.critical(
        'FIREFLIES_API_KEY is required but not provided - this is a critical configuration error',
      );
  ...
  async listTranscripts(
    options: FirefliesTranscriptListOptions = {},
    ...
    while (transcripts.length < maxRecords) {
      ...
      const page = await this.executeTranscriptListQuery(
        baseQuery,
        pageVariables,
      );
      const normalized = page
        .map((item) => {
          ...
          return {
            ...
            summary_status: (
              item.meeting_info as { summary_status?: string } | undefined
            )?.summary_status,
          };
        })
        .filter((item) => {
  ...
  async fetchMeetingData(
    ...
    const queryToUse = isPremiumPlan
      ? businessQuery
      : plan === FIREFLIES_PLANS.PRO
        ? proQuery
        : freeQuery;
    ...
    const planFeatures = {
      ...
      [FIREFLIES_PLANS.ENTERPRISE]:
        'full access including analytics, video_url',
    };
    ...
    try {
      ...
      const requiresBusiness = message
        .toLowerCase()
        .includes('business or higher');
      ...
      if (planError) {
        if (isPremiumPlan) {
          logger.warn(
            `Plan limitation detected (configured: ${plan}), falling back to pro query`,
          );
          try {
            return await this.executeTranscriptQuery({
              meetingId,
            ...
            if (
              proMessage.toLowerCase().includes('plan') ||
              proMessage.includes('Cannot query field')
            ) {
              logger.warn(
                'Pro query also failed, falling back to minimal free query',
              );
          ...
          logger.warn(
            `Pro plan query failed (${requiresBusiness ? 'requires Business+' : 'unknown restriction'}), falling back to free query`,
          );
          return this.executeTranscriptQuery({
            meetingId,
            query: freeQuery,
  ...
  async fetchMeetingDataWithRetry(
    ...
    if (config.strategy === 'immediate_only') {
      ...
      const meetingData = await this.fetchMeetingData(meetingId, {
        timeout: 10000,
        plan,
      });
    ...
    logger.debug(
      `fetching meeting ${meetingId} (strategy: immediate_with_retry, maxAttempts: ${config.retryAttempts})`,
    );
    ...
    for (let attempt = 1; attempt <= config.retryAttempts; attempt++) {
      try {
        ...
        logger.debug(
          `attempt ${attempt}/${config.retryAttempts}: summary ready=${ready}`,
        );
        ...
        if (attempt < config.retryAttempts) {
          ...
          logger.debug(
            `summary not ready, waiting ${delayMs}ms before retry ${attempt + 1}`,
          );
        ...
        logger.error(
          `attempt ${attempt}/${config.retryAttempts} failed: ${errorMsg}`,
        );
  ...
  private async executeTranscriptQuery({
    ...
    try {
      ...
      if (json.errors && json.errors.length > 0) {
        throw new Error(
          `Fireflies API error: ${json.errors[0]?.message || 'Unknown error'}`,
        );
      }
      ...
      if (!transcript) {
        throw new Error(
          'Invalid response from Fireflies API: missing transcript data',
        );
      }
  ...
  private extractAllParticipants(
    ...
    logger.debug(
      'meeting_attendees field:',
      JSON.stringify(transcript.meeting_attendees),
    );
    logger.debug(
      'speakers field:',
      (transcript.speakers as Array<{ name: string }>)?.map((s) => s.name),
    );
    logger.debug(
      'meeting_attendance field:',
      (transcript.meeting_attendance as Array<{ name: string }>)?.map(
        (a) => a.name,
      ),
    );
      ...
      transcript.participants.forEach((participant: string) => {
        ...
        parts.forEach((part) => {
          ...
          if (isDuplicate(name, email)) {
            logger.debug(
              `Skipping duplicate participant: "${name}" <${email}>`,
            );
            return;
          }
    ...
    if (
      ...
      Array.isArray(transcript.meeting_attendees)
    ) {
      transcript.meeting_attendees.forEach(
    ...
    if (
      ...
      Array.isArray(transcript.meeting_attendance)
    ) {
      transcript.meeting_attendance.forEach(
        (attendance: Record<string, unknown>) => {
          const name = (attendance.name || '') as string;

          if (isEmail(name) || name.includes(',')) {
            logger.debug(
              `Skipping attendance with email/list as name: "${name}"`,
            );
            return;
          }
    ...
    if (organizerEmail) {
      const existsWithEmail = participantsWithEmails.some(
        (p) => p.email.toLowerCase() === organizerEmail.toLowerCase(),
      );
      ...
      if (!existsWithEmail) {
        ...
        if (transcript.speakers && Array.isArray(transcript.speakers)) {
          const potentialOrganizerSpeaker = transcript.speakers.find(
            (speaker: Record<string, unknown>) => {
              const name = ((speaker.name || '') as string).toLowerCase();
              return emailNameVariations.some(
                (variation) =>
        ...
        if (
          ...
          const potentialOrganizerAttendance =
            transcript.meeting_attendance.find(
              (attendance: Record<string, unknown>) => {
        ...
        if (organizerName) {
          participantsWithEmails.push({
            name: organizerName,
            email: organizerEmail,
          });

          const nameIndex = participantsNameOnly.findIndex(
            (p) =>
    ...
    logger.debug(
      'With emails:',
      participantsWithEmails.length,
      JSON.stringify(participantsWithEmails),
    );
    logger.debug(
      'Name only:',
      participantsNameOnly.length,
      JSON.stringify(participantsNameOnly),
    );
  ...
  private transformMeetingData(
    ...
    const analytics = transcript.analytics as
      | Record<string, unknown>
      | undefined;
    const sentiments = analytics?.sentiments as
      | Record<string, number>
      | undefined;
    const categories = analytics?.categories as
      | Record<string, number>
      | undefined;
    const speakersAnalytics = analytics?.speakers as
    ...
    const sentences = rawSentences?.map((s) => ({
      ...
      ai_filters: s.ai_filters as
        | { task?: boolean; question?: boolean; sentiment?: string }
        | undefined,
    }));
    ...
    return {
      ...
      summary: {
        ...
        transcript_chapters: summary?.transcript_chapters as
          | string[]
          | undefined,
      },
      analytics:
        sentiments || categories || speakers
          ? {
      ...
      summary_status:
        (meetingInfo?.summary_status as string) ||
        (transcript.summary_status as string) ||
        undefined,
    };
  }
  ...
  private async executeTranscriptListQuery(
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      throw new Error(
        `Fireflies transcripts request failed: ${response.status} ${errorBody}`,
      );
    }
