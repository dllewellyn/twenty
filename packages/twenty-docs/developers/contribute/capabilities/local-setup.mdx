...
Twenty is now **Firebase-native**. Core CRM functionality no longer requires a local PostgreSQL or Redis instance. Instead, it utilizes the Firebase Emulator Suite for Authentication and Firestore.

In a new terminal window, start the emulators:
...
- **Firestore**: NoSQL database for records and metadata (Port 8080)
- **Authentication**: User management and login flows (Port 9099)
- **Emulator UI**: A web dashboard to inspect data at [http://localhost:4000](http://localhost:4000)

<Info>
To persist data across emulator restarts, use `yarn firebase:emulators:import` and `yarn firebase:emulators:export`.
</Info>
...
To build Twenty and its workspaces, run:
```bash
yarn
```
...
This command runs both the `twenty-server` and `twenty-front` packages.
...
The following steps are for legacy development environments that still require PostgreSQL and Redis.

## [Legacy] Step 3: Set up a PostgreSQL Database
...
<Tabs>
  ...
  <Tab title="Windows (WSL)">
    **Option 1:** To provision your Postgresql locally:
    [Postgresql Installation](https://www.postgresql.org/download/linux/)
    ```bash
      psql postgres -c "CREATE DATABASE \"default\";" -c "CREATE DATABASE test;"
    ```
  </Tab>
...
## [Legacy] Step 4: Set up a Redis Database (cache)
Twenty requires a redis cache for the legacy BullMQ-based worker.
